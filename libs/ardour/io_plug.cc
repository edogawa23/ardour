...
IOPlug::IOPlug (Session& s, PluginInfoPtr pip, bool pre)
	: SessionObject (s, "io-plug")
	, _pre (0)
	, _state (0)
{
	g_atomic_int_set (&_stat_reset, 0);

	string io_name = "sth-uniq";

	_input.reset (new IO (s, io_name, IO::Input));
	_output.reset (new IO (s, io_name, IO::Output));
...
bool
IOPlug::set_name (std::string const& new_name)
{
	bool ret = true;
...
bool
IOPlug::load_plugin (PluginInfoPtr pip)
{
	assert (pip);
	 _plugin = pip->load (_session);
	 if (!_plugin) {
		 return false;
	 }
...
bool
IOPlug::load_preset (Plugin::PresetRecord)
{
	return false;
}
